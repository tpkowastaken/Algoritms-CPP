...
class heap{
    private:
        ...
        */
        heap_node *swap(heap_node *firstNode, heap_node *secondNode){//we could also transform just the value but that wont keep the same value to a pointer
            if(!firstNode){
        ...
        void bubbleDown(heap_node *nodeToBeBubbledDown){
            ...
            while(nodeToBeBubbledDown->childLeft || nodeToBeBubbledDown->childRight){
                ...
                || nodeToBeBubbledDown->childRight->value < nodeToBeBubbledDown->value)){
                    nodeToBeBubbledDown = swap(nodeToBeBubbledDown, nodeToBeBubbledDown->childLeft->value < nodeToBeBubbledDown->childRight->value ? nodeToBeBubbledDown->childLeft : nodeToBeBubbledDown->childRight);
                }
                else if(nodeToBeBubbledDown->value > nodeToBeBubbledDown->childLeft->value){
                    nodeToBeBubbledDown = swap(nodeToBeBubbledDown, nodeToBeBubbledDown->childLeft);
                }
                else {
                    break;
                }
        ...
        */
        void bubbleUp(heap_node *nodeToBeBubbledUp){
            ...
            while(nodeToBeBubbledUp->parent && nodeToBeBubbledUp->parent->value > nodeToBeBubbledUp->value){
                nodeToBeBubbledUp = swap(nodeToBeBubbledUp, nodeToBeBubbledUp->parent);
            }
        }
        /*
        ...
        */
        int extractMin(){
            ...
            if(lastLineSize == 1 && !head->childRight){
                delete last;
                ptr->childLeft = nullptr;
                ptr->childRight = nullptr;
                ptr->parent = nullptr;
                return value;
            }
            if(lastLine[currentLastNode]->childLeft == last){
                lastLine[currentLastNode]->childLeft = nullptr;
                currentLastNode--;
            }
            else if(lastLine[currentLastNode]->childRight == last){
