...
class heap{
    private:
        int *x;//the array that the heap will be stored in
        ...
        void bubbleUp(int i){
            while(parentOf(i) != -1 && x[parentOf(i)] > x[i]){
                i = swap(parentOf(i), i);
            }
        }
        void bubbleDown(int i){
            while(rightChildOf(i) < n && (x[leftChildOf(i)] < x[i] || x[rightChildOf(i)] < x[i])){
                i = swap(x[leftChildOf(i)] < x[rightChildOf(i)] ? leftChildOf(i) : rightChildOf(i), i);
            }
        }
        ...
        */
        void sort(){
            ...
            for(int i = currentIndex; i>=0; i--){
                for(int g = i;
                g > 0 && x[parentOf(g)] > x[g] //as long as we are in range and parent is smaller
                && (leftChildOf(parentOf(g)) == g //and as long as we are either leftchild or smaller than leftchild
                || x[leftChildOf(parentOf(g))] > x[g]);g = parentOf(g)){//modified bubbleup
                    swap(parentOf(g), g);
                    bubbleDown(g);
                }
            }
