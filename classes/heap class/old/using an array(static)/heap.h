...
class heap{
    private:
        ...
        int empty = INT_MAX;//integer limit
    public:
        heap(int size){
            if(!size){
                throw myException("heap was tried to be built with size 0 -- heap::heap");
            }
            n = size;
        ...
        void bubbledown(int i){
            int value = x[i];
            while((x[i*2] != empty /*hodnota pod vlevo*/ || x[i*2+1] != empty /*hodnota pod vpravo*/ ) && i*2+1 <= n+1){//dokud existuje aspon jedna hodnota pod a nesnazi se to pristoupit k hodnote mimo pole
                if(i*2+1 >= n+1 && i*2 < n+1 && x[i*2] != empty && x[i*2] < value){
                    i = move(i*2,i);
                    break;
                }
        ...
        void sort(){
            int value;
            for(int i = l;i-1;i--){//counting from the last to first
                value = x[i];//getting the value so that it can write it afterwards
                if(i%2 == 1){//if it's the right child
                    int g = i;//basically bubbleup
                    while(g){
                        if(x[g-1] <= x[g] || x[g/2] <= x[g] || g-1 == 0)break;
                        x[g] = x[g/2];
                        bubbledown(g);//but with bubbledown implemented as well
                        g /= 2;
                    }
                    x[g] = value;//here!
                }
